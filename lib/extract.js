...
export async function extractPainPoints(market, items, { onProgress } = {}) {
  ...
  const responses = await mapWithConcurrency(batches, CONCURRENCY, async (batch) => {
    try {
      ...
      // run reports that rather than pretending it saw everything. But it does
      // need to be diagnosable: swallowing this silently once hid 14 of 15
      // batches failing behind an honest-looking "insufficient evidence".
      console.error(`[extract batch failed] ${error.message}`);
      return null;
    } finally {
      completed += 1;
