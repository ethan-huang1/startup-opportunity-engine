 ...
 *
 * Signing up grants exactly one thing: the ability to read saved analyses.
 * It can never grant admin — admin is derived only from the ADMIN_EMAILS
 * environment variable, server-side, on every request (see lib/access.js).
 * There is no role column an account could set on itself.
 */
...
export const auth = betterAuth({
  ...
  emailAndPassword: {
    enabled: true,
    // Open self-signup, deliberately. A new account can read saved reports
    // and nothing else: generation is admin-only and additionally refuses to
    // run in production at all (lib/access.js), so a signup cannot cost
    // anything beyond its own row.
    disableSignUp: false,
    minPasswordLength: 8,
