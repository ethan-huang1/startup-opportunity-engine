...
// Cached per warm instance: `which` is cheap, but there is no reason to pay
// it on every request when the answer cannot change mid-instance.
let availability = null;

/**
 * Whether the `claude` CLI is actually runnable here. Used as a fast,
 * outbound-request-free preflight — the pipeline should never scrape a
 * source only to fail later at the first Claude call.
 */
export function isClaudeAvailable() {
  if (availability) return availability;
  availability = new Promise((resolve) => {
    const check = spawn('which', ['claude'], { stdio: 'ignore' });
...
/** Test-only: forces the next isClaudeAvailable() call to re-check. */
export function _resetClaudeAvailabilityForTests() {
  availability = null;
}
