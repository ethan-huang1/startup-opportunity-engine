/**
 * Server-side authorization and kill-switch config.
 *
 ...
 *
 * There are no admin-only *routes* any more. Reading is public, and
 * generation is public-but-metered (lib/usage.js); admin is now one thing
 * only — an exemption from that meter, so the maintainer can test.
 */
 ...
 *   1. this switch, which fails closed on anything but the literal "true";
 *   2. isClaudeAvailable(), which refuses when there is no CLI to run
 *      (the usual case in a serverless runtime);
 *   3. the per-network rate limit and the system-wide in-flight guard;
 *   4. the three-search allowance in lib/usage.js, counted in Postgres,
 *      which every caller but an admin has to get past.
 *
...
/**
 * Basic per-key request cap, in memory. A brake on request *rate*; the
 * durable cap on how much anyone may generate is lib/usage.js, which is in
 * Postgres precisely because this is not.
 * ponytail: resets on cold start and isn't shared across warm instances.
 * A Postgres-backed limiter would be needed for a real cross-instance
 * guarantee.
 */
